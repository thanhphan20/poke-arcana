...
- Add a `tarotArtUrl(arcana)` helper alongside `spriteUrl` that derives a **local** `/tarot/…` path from arcana identity — `m{majorNumber}.webp` for majors, `{suit-letter}{rankIndex+1}.webp` for minors (c/s/w/p). No env var, no CDN base, no full URLs in content JSON.
- Commit the 78 Rider-Waite-Smith images (22 major + 56 minor) to `public/tarot/`, fetched once by a repo-run script `scripts/download-tarot-images.ts` from `github.com/krates98/tarotcardapi`, downscaled + re-encoded to optimized WebP, and saved under the derivable `mNN.webp` / `{suit}{NN}.webp` scheme.
...
Non-goals: collapsing the two-tap flow, changing the Pokémon-front card, commissioning custom Pokémon-arcana art, responsive `srcset` / multi-resolution variants, extending the Pokémon dataset (individual stats/abilities), and adopting the richer JSON metadata fields (fortune_telling, light/shadow, etc.) — the typed `tarot-metadata.ts` stays the single source of truth.
...
- **Assets**: 78 optimized WebP images (~2.8MB, downscaled to ≤560px @ q75) committed to `public/tarot/`, served same-origin like `favicon.svg`. `public/tarot-images.json` was evaluated and deleted — 139KB shipped to visitors but read by nothing at runtime.
