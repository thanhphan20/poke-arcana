...
- **No verbatim Pokedex trivia**: real output from the above requirement over-corrected into citing raw flavor-text facts to prove the link ("Metagross's four brains linked by a cable", "the Remoraid attached to it", quoted phrases like "a cotton-fluff hat") — accurate, but opaque to a reader with no Pokemon background, and it reads as trivia rather than tarot prose. The prompt now requires translating flavor text into a plain-language temperament (calm, restless, protective, playful, watchful, methodical) *before* using it, and explicitly bans verbatim trivia/anatomy/lore and quoted flavor text. Goal: the interpretation must stand alone as tarot prose for a reader who has never played a Pokemon game.
- **Generalized past specific banned phrases**: a second round of real output (Cacnea → "predatory strategies", Silcoon → "patient evolution") showed the previous fix was read too literally as a list of specific phrases to avoid, not a principle to generalize from. Two failure modes, two fixes: (1) `evolve`/`evolution` is banned outright, even as a growth metaphor, because it's literally this Pokemon's own game mechanic and reads as an inside reference rather than a universal image — replaced with `transforms`/`unfolds`/`grows into`/`blossoms`. Same treatment for `battle`/`level`/`stats`/`ability`/`move`/`type`/`catch`/`breed`/`hatch`. (2) The prompt now explicitly requires a warm, empowering register — a Pokemon's defensive/hunting nature should surface as "sharp instinct" or "fierce self-protection", not "predatory", which reads as an accusation aimed at the querent rather than a strength being named. Verified against the exact spread reported (Parasect/Cacnea/Silcoon, "next month" financial question): re-run output uses "guarded nature", "defensive", "blossom into abundance" — no evolution/predatory language.
